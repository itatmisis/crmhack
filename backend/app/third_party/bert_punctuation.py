...
class Bert_punctuation(object):
    def __init__(self):
        self.model_file = str(DATA / "bert_punctuation.tar.gz")
        self.vocab_file = str(DATA / "vocab.txt")
    ...
    def predict(self, texts):
        ...
        for i, words in enumerate(words_all):
            ...
            choice_list = words.replace('–', '-').replace('—', '-').replace(',', ' ,').replace('.', ' .').replace('!',
                                                                                                                  ' !').replace(
            ...
            for j, p in enumerate(pos):
                        ...
                        if pos[ii] == 'NOUN':
                            for all_cases_j in all_cases[j]:
                                for all_cases_z in all_cases[ii]:
                                    ...
                                    if (all_cases_j.tag.case == all_cases_z.tag.case) and (
                                            choice_list[j] != 'свет' or choice_list[ii] != 'заря'):
                                        odnorod = True
                            if odnorod:
                ...
                if (p == 'CONJ' or choice_list[j] == 'да' or choice_list[j] == 'ни') and j > 0 and pos[j - 1] != 'CONJ':
                    ...
                    elif choice_list[j] != 'ни' or (
                            choice_list[j] == 'ни' and (j == 0 or choice_list[j - 1] != 'свет') and (
                            j == len(pos) or choice_list[j + 1] != 'свет')):
                        # print('Союз',choice_list[j],choice_list[j-1],choice_list[j+1])
                        for pb in par_b:
                            if (j != 0 and choice_list[j - 1] == pb[0]) and (
                                    j != len(pos) and choice_list[j + 1] == pb[1]):
                                bad_par = True
            ...
            words = ' '.join(choice_list).replace(' ,', ',').replace(' .', '.').replace(' !', '!').replace(' ?',
                                                                                                           '?').replace(
